 ...
 */
class EgmulticatproductsBloc extends ObjectModel
{
    /** @var int id_eg_advanced_menu_bloc */
    public $id_eg_multicat_products_bloc;
	...
	public static $definition = array(
		'table' => 'eg_multicat_products_bloc',
		'primary' => 'id_eg_multicat_products_bloc',
        'multilang_shop' => true,
        'multilang' => true,
        'fields' => array(
            'position' =>   array('type' => self::TYPE_INT, 'validate' => 'isUnsignedInt'),
            'id_eg_multicat_products' =>   array('type' => self::TYPE_INT, 'validate' => 'isUnsignedInt'),
            'position' =>   array('type' => self::TYPE_INT, 'validate' => 'isUnsignedInt'),
    ...
    public static function getMaxPosition()
    {
        ...
        $query->from('eg_multicat_products_bloc', 'eg');
    ...
    {
        $query = new DbQuery();
        $query->select('eg.`id_eg_multicat_products_bloc`, eg.`position`');
        $query->from('eg_eg_multicat_products', 'eg');
        ...
        foreach ($tabs as $tab) {
            if ((int) $tab['id_eg_multicat_products_bloc'] == (int) $this->id) {
                $moved_tab = $tab;
            }
        }
        ...
        return (Db::getInstance()->execute('
            UPDATE `'._DB_PREFIX_.'eg_multicat_products_bloc`
            ...
            WHERE `id_eg_multicat_products_bloc` = '.(int)$moved_tab['id_eg_multicat_products_bloc']));
    }
    ...
    $sql = 'SELECT * FROM `'._DB_PREFIX_.'eg_multicat_products_bloc` l left join '._DB_PREFIX_.'eg_multicat_products_bloc_lang ll on (l.id_eg_multicat_products_bloc = ll.id_eg_multicat_products_bloc)';
    ...
    public static function getNbr($id = null)
    {
   
    $sql = 'SELECT nbr FROM `ps_eg_multicat_products_bloc`';
    $data = Db::getInstance()->executeS($sql);
    if (empty($data)) {
        return [];
    }
    ...
    $query->from('eg_multicat_products_bloc', 'b');
    ...
    $query = new DbQuery();
    $query->select('c.`id_eg_multicat_products`');
    $query->from('eg_multicat_products_bloc', 'c');
